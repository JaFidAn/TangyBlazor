...
namespace TangyWeb_Client.Service
{
    public class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<SignInResponseDTO> Login(SignInRequestDTO signInRequest)
        {
            ...
            var response = await _httpClient.PostAsync("api/accounts/signin", bodyContent);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                ((AuthStateProvider)_authStateProvider).NodifyUserLoggedIn(result.Token);
        ...
        public async Task Logout()
        {
            ...
            ((AuthStateProvider)_authStateProvider).NodifyUserLogout();
        ...
        public async Task<SignUpResponseDTO> RegisterUser(SignUpRequestDTO signUpRequest)
        {
            ...
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _httpClient.PostAsync("api/accounts/signup", bodyContent);
            var contentTemp = await response.Content.ReadAsStringAsync();
